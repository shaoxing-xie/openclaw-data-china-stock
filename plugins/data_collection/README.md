...
- 所有对外 `tool_*` 建议统一输出 `source_id/source_raw/source_stage`。
- 全局指数与 A50 工具已补充可观测字段：`elapsed_ms`、`quality`、`degraded_reason`、`failure_code`、`attempts`。
- A50 实时链路新增短 TTL 热缓存（默认 30s），响应中包含 `cache_hit` 与 `cache_age_ms`，用于验证热路径命中率。
- `source_id` 的 canonical 集合、别名映射和新增数据源的文档门禁规则，统一维护在：
  - [DATA_SOURCE_REGISTRY.md](./DATA_SOURCE_REGISTRY.md)
- 贡献约束：
  - 新增数据源或新增 source 分支时，必须同步更新 `DATA_SOURCE_REGISTRY.md`；
  - 新增 `tool_*`（`config/tools_manifest.yaml`）时，必须更新 registry 中的 tool/domain 映射说明。
...
2. **数据源与限频**：公开接口可能变更或限流；生产环境建议加重试、缓存与降级。
   - `tool_fetch_global_index_spot` 建议按源控制最小间隔：`yfinance >= 0.3s`、`fmp >= 0.8s`，并采用快慢路径与有限重试预算。
   - `tool_fetch_a50_data` 实时链路仅使用期货源，禁止指数替代期货；实时不可得时必须显式降级标记，不得伪装实时成功。
